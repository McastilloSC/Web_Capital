...
const FormEditarProyecto = ({ infoProyecto, setProyectos, setProyectosPagination }) => {
    ...
    const formik = useFormik({
        ...
        initialValues: {
            empresa: infoProyecto.id_empresa,
            ejecutivo: infoProyecto.id_ejecutivo,
            ingeniero: infoProyecto.id_ingeniero,
            proyecto: infoProyecto.proyecto,
            tipo_proyecto: infoProyecto.tipo_proyecto,
            division: infoProyecto.id_division,
            producto: infoProyecto.producto,
            mercancia: infoProyecto.mercancia,
            to_anual: infoProyecto.to_anual,
        },
        onSubmit: (values) => {
            onActualizarProyecto(values)
        },
    })

    const onActualizarProyecto = async (values) => {
        setLoading(true)
        try {
            const { data } = await toast.promise(axios.patch(`/pipeline/proyectos/${infoProyecto.id_proyecto}/cliente/${idCliente}`, values), {
                ...
                <div className="grid grid-cols-1 gap-5">
                    <div className="form-control w-full">
                        <label className="label">
                            ...
                            { errores && <span className="label-text-alt text-red-500">{errores}</span> }
                        </label>
