...
const FormEditaLlamada = ({ infoLlamada, setLlamadas, setLlamadasPagination}) => {
    ...
    const formik = useFormik({
        ...
        initialValues: {
            duracion: infoLlamada.duracion,
            fecha: infoLlamada.fecha_llamada,
            motivo: infoLlamada.motivo,
            observaciones: infoLlamada.motivo,
            oportunidad: infoLlamada.oportunidad == 1 ? true : false,
            cita: infoLlamada.cita == 1 ? true : false
        },
        onSubmit: (values) => {
            onEditarLlamada(values)
        },
    })

    const onEditarLlamada = async (values) => {
        setLoading(true)
            try {
                const { data } = await toast.promise(axios.patch(`/pipeline/llamadas/${infoLlamada.id_llamada}`, values), {
                    pending: 'Actualizando Llamada',
                    success: 'Llamada Actualizada'
                });
            ...
            <div className="grid grid-rows-1 gap-5">
                ...
                <div className="grid grid-cols-3 gap-5">
                    ...
                    <div className="w-full flex justify-center items-center">
                        ...
                        <input checked={formik.values.oportunidad} name="oportunidad" onChange={formik.handleChange} value={formik.values.oportunidad} type="checkbox" className="checkbox border-2 ml-5"/>
                    </div>
                    <div className="w-full flex justify-center items-center">
                        <label className="font-semibold">¿ Genero Cita ?</label>
                        <input checked={formik.values.cita} name="cita" onChange={formik.handleChange} value={formik.values.cita} type="checkbox" className="checkbox border-2 ml-5"/>
                    </div>
                </div>

                <div className="grid grid-cols-1">
                    <button disabled={loading} type="submit" className="btn btn-sm btn-secondary w-full">Actualizar Llamada</button>
                </div>
            </div>
        </form>
    )
...
export default FormEditaLlamada