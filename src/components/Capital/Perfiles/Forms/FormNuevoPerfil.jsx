...
const FormNuevoPerfil = ({  setPerfiles, setPerfilesPagination }) => {
    ...
    const formik = useFormik({
        ...
        initialValues: {
            ...
            menu_perfiles: false,
            menu_usuarios: false,
            registro_empleado: false,
            editar_empleado: false,
            baja_empleado: false,

            registro_incidencia: false,
            editar_incidencia: false,
            eliminar_incidencia: false,

            registrar_usuarios: false,
            editar_usuarios: false,
            eliminar_usuarios: false,

            incidencias_generales: false,
            incidencias_monto: false,
            tiempo_extra: false,
        },
        onSubmit: (values) => {
            onRegistroPerfil(values)
        },
    })

    const onRegistroPerfil = async (values) => {
        setLoading(true)
        try {
            const { data } = await toast.promise(axios.post('/capital/perfiles', values), {
                pending: 'Registrando Perfil',
                success: 'Perfil Registrado',
            });
            ...
            <div className="grid grid-rows-1 gap-5">
                <div className="grid grid-cols-12 gap-5">
                    ...
                    <div className="col-span-3">
                        ...
                        <button disabled={loading} type='submit' className="btn btn-sm btn-secondary w-full"> Regisrar Perfil</button>
                    </div>
                </div>
...
export default FormNuevoPerfil