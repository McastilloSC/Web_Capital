...
const FormNuevoEmpleado = ({ setEmpleados, setEmpleadosPagination }) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            onRegistroEmpleado(values)
        },
    })

    const onRegistroEmpleado = async (values) => {
        setErrores({})
        try {
            setLoading(true)
            const { data } = await toast.promise(axios.post('/capital/empleado', values, {
                headers: {
        ...
        <form onSubmit={formik.handleSubmit}>
            <div className='grid grid-rows-1 gap-5'>
                ...
                <div className='grid grid-cols-1 gap-5 mt-3'>
                    ...
                    <button disabled={loading} type='submit' className='btn btn-sm btn-secondary'>Registrar Empleado</button>
                </div>
            </div>
        </form>    
    )
