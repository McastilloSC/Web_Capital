...
const FormEditarTiempoExtra = ({ tiempoExtraInfo, setTiempoExtra, setTiempoExtraPagination }) => {
    ...
    const formik = useFormik({
        ...
        initialValues: {
            fecha: tiempoExtraInfo.fecha,
            tiempo: parseInt(tiempoExtraInfo.tiempo),
        },
        onSubmit: (values) => {
            onUpdateTiempoExtra(values)
        },
    })

    const onUpdateTiempoExtra = async (values) => {
        setLoading(true)
        try {
            const { data } = await toast.promise(axios.patch(`/capital/tiempo_extra/update/${tiempoExtraInfo.id_tiempo_extra}`, values), {
                pending: 'Actualizando Tiempo Extra',
                success: 'Tiempo Extra Actualizado',
            })
            ...
            <div className='grid grid-rows-1 my-2'>
                <div className='grid grid-cols-3 gap-5'>
                    ...
                    <div className="form-control w-full">
                        ...
                        <button disabled={loading} type='submit' className='btn btn-sm btn-secondary'>Actualizar Tiempo Extra</button>
                    </div>
                </div>
            </div>
        </form>
...
export default FormEditarTiempoExtra