...
const FormNuevoEquipo = ({ setEquipos, setEquiposPagination }) => {
    ...
    const registroSchema = Yup.object().shape({         
        ...
        imagenes: Yup.array().min(1, 'Minimo una imagen').max(5, 'Maximo 5 Imagenes')
    })
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            onRegistroEquipo(values)
        },
    })

    const onRegistroEquipo = async (values) => {
        setLoading(true)
        setErrores('')
        try {
            const { data } = await toast.promise(axios.post('/inventario/equipos', values, {
                headers:{
                    'Content-Type':'multipart/form-data'
                }
            }), {
                success: 'Equipo Registrado Correctamente'
            })
            ...
            <div className="grid grid-rows-1 gap-5">
                ...
                <div className="grid grid-cols-4 gap-5">
                    ...
                    <div className="form-control w-full ">
                        ...
                        <input disabled={loading} type="text" name='modelo' value={formik.values.modelo} onChange={formik.handleChange} placeholder="...." className="border-2 border-gray-300 input input-sm input-bordered w-full " />
                    </div>
                    <div className="form-control w-full ">
                        <label className="label">
                        ...
                        <input disabled={loading} type="text" name='marca' value={formik.values.marca} onChange={formik.handleChange} placeholder="...." className="border-2 border-gray-300 input input-sm input-bordered w-full " />
                    </div>
                    <div className="form-control w-full ">
                        <label className="label">
                        ...
                        <input disabled={loading} type="text" name='serie' value={formik.values.serie} onChange={formik.handleChange} placeholder="...." className="border-2 border-gray-300 input input-sm input-bordered w-full " />
                    </div>
                </div>
                ...
                <div className="form-control w-full ">
                    <label className="label">
                        {errores && (
                            <span className="label-text-alt text-red-500">{errores}</span>
                        )}
                    </label>
